...
def cpu(id, value, period=100000):
    ...
    path = '/sys/fs/cgroup/cpu/kubepods.slice/kubepods-pod' + id + '.slice/cpu.cfs_quota_us'
    f = open(path,"r")
    original = int(f.read())
...
def network(id, value):
    ...
    path = '/sys/fs/cgroup/cpu/kubepods.slice/kubepods-pod' + id + '.slice/net_cls.classid'
    # f = open(path,"r")
    # original = int(f.read())
    with open(path, "r+") as f:
        data = f.read()
...
def blkio(id, value):
    ...
    path = '/sys/fs/cgroup/cpu/kubepods.slice/kubepods-pod' + id + '.slice/blkio.weight'
    with open(path, "r+") as f:
        data = f.read()
        f.seek(0)
        f.write(value)
